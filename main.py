...
def load_env_file(path=".env"):
    """Load simple KEY=VALUE pairs for local development without requiring extra packages."""
...
conn = psycopg2.connect(
    database=os.getenv("DB_NAME", "Wall-Street-Admin"),
...
def signup():
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO customers (f_name, l_name, user_id, pass, email, contact) VALUES (%s, %s, %s, %s, %s, %s)",
            (f_name, l_name, user_id, user_password, e_mail, contact)
        )
        conn.commit()
...
def signup_hod():
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO adminusers (f_name, l_name, user_id, user_password, e_mail, phone_number, branch_code, user_type) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)",
            (f_name, l_name, user_id, user_password, e_mail, phone_number, branch_code, "HOD")
        )
        conn.commit()
...
def open_branch():
    if request.method == 'POST':
        ...
        try:
            cur = conn.cursor()
            cur.execute("INSERT INTO branch VALUES (%s, %s)", (branch_code, city))
            conn.commit()
...
def signin():
    ...
    cur = conn.cursor()
    cur.execute("SELECT GET_customers_PASS(%s)", (user_id,))
    result = cur.fetchall()
    conn.commit()
    cur = conn.cursor()
    ...
    if (result and user_password == result[0][0]):
...
def signinadmin():
    ...
    # getting password match of user_id
    cur = conn.cursor()
    cur.execute("SELECT Get_PASS(%s)", (user_id,))
    result = cur.fetchall()

    if not result or user_password != result[0][0]:
        error = "invalid password or user id"
        return render_template('index.html', error=error)

    cur.execute("CALL update_current_user(%s)", (user_id,))
    conn.commit()
    cur = conn.cursor()
    cur.execute(
        "SELECT user_name FROM current_login_user WHERE user_id = %s", (user_id,))
    current_user = cur.fetchone()
    u_name = current_user[0] if current_user else ""
    print(u_name)

    cur = conn.cursor()
    cur.execute("SELECT GET_type(%s)", (user_id,))
    u_type = cur.fetchall()
    u_types = [('CEO', 'HOD')]
    if (u_type and u_type[0][0] == u_types[0][0]):
        return render_template('ceo.html', u_name=u_name)
    elif (u_type and u_type[0][0] == u_types[0][1]):
...
def add_products():
    if (request.method == 'POST'):
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT add_products(%s, %s, %s, %s, %s, %s, %s)",
            (category, price, pro_type, pro_location, address, p_size, details)
        )
        conn.commit()
...
def invoice():
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute("SELECT branch_code FROM adminusers WHERE user_id = (SELECT user_id FROM current_login_user)")
        branch = cur.fetchone()[0]
        cur.execute("CALL add_invoice(%s, %s, %s)", (branch, income, expences))
        print(branch)
        conn.commit()
...
def PaySal():
    ...
    if request.method == 'GET':
        ...
        cur.execute(
            "SELECT branch_code FROM adminusers WHERE user_id = %s", (current_user_id,))
        b_id = cur.fetchall()
        conn.commit()
        id = b_id[0][0]
        print('branch', id)
        cur = conn.cursor()
        cur.execute("CALL paysal(%s)", (id,))
        conn.commit()
...
def delete_hod():
    if request.method == 'POST':
        ...
        if str(user_id) == "1":
            ...
            cur = conn.cursor()
            cur.execute(
                "DELETE FROM adminusers WHERE user_id = %s", (user_id,))
            conn.commit()
...
def add_emp():
    if request.method == 'POST':
        ...
        cur = conn.cursor()
        cur.execute("INSERT INTO employees VALUES (%s, %s, %s, %s)", (name, eid, br, sal))
        conn.commit()
